...
// ════════════════════════════════════════════════════════════════════════
// "MERAK KAPISI" LANDING — 2026-08-14'e kadar app/page.tsx'in GÖVDESİYDİ.
//
// O tarihte ana sayfa akışa çevrildi (tek ana sayfa kararı: ziyaretçi
// pazarlama sayfası yerine doğrudan içeriğe düşsün). Bu dosya SİLİNMEDİ,
// buraya taşındı — geri dönmek isteyen için app/page.tsx'te tek satır:
//
//     return <LandingPage />;
//
// SAF SUNUM: JSON-LD ve metadata BİLEREK burada değil, sayfada durur —
// hangi rota `/`yi servis ediyorsa marka işaretlemesi onun sorumluluğu.
// ════════════════════════════════════════════════════════════════════════
export default function LandingPage() {
  ...
  return (
    <div className={s.lp}>
      ...
      {/* ══════════ 8 · ARŞİV — kategoriye gruplu, sıfır JS ══════════ */}
            ...
            <div key={c.cat} id={`raf-${c.cat.toLocaleLowerCase('tr-TR')}`} className={s.anchor}>
              ...
              <div className={s.rows}>
                {ARTICLES.filter((a) => a.category === c.cat).map((a) => (
                  // prefetch={false}: arşiv makalelerin TAMAMINI listeler; viewport
                  // prefetch'i arşive kaydıran her ziyaretçiye ~815KB RSC indirtiyor
                  // ve her deploy sonrası onlarca fonksiyon çağrısı patlatıyordu
