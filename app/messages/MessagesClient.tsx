...
export default function MessagesClient({ conversations: initialConvs, me }: Props) {
  ...
  for (const m of messages) {
    ...
    if (day !== lastDay) {
      msgElements.push(<div key={`day-${m.created_at}`} style={{ textAlign: 'center', fontSize: '0.72rem', color: 'var(--color-text-muted)', margin: '8px 0', display: 'flex', alignItems: 'center', gap: 8 }}><div style={{ flex: 1, height: 1, background: 'var(--color-hover)' }} />{day}<div style={{ flex: 1, height: 1, background: 'var(--color-hover)' }} /></div>);
      lastDay = day;
    }
      ...
      <div key={m.id} style={{ display: 'flex', alignItems: 'flex-end', gap: 6, maxWidth: '72%', alignSelf: mine ? 'flex-end' : 'flex-start', flexDirection: mine ? 'row-reverse' : 'row' }}>
        ...
        {m.media_url ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: mine ? 'flex-end' : 'flex-start' }}>
            ...
            {m.content && (
              <div style={{ padding: '9px 14px', borderRadius: 18, fontSize: '0.88rem', lineHeight: 1.5, wordBreak: 'break-word', whiteSpace: 'pre-wrap', ...(mine ? { background: 'var(--color-accent)', color: '#0f0e0d', borderBottomRightRadius: 4 } : { background: 'var(--color-hover)', color: 'var(--color-text)', borderBottomLeftRadius: 4 }) }}>
                {m.content}
              </div>
            )}
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 4, alignItems: mine ? 'flex-end' : 'flex-start' }}>
            {m.story?.media_url && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, opacity: 0.85 }}>
                <span style={{ fontSize: '0.66rem', color: 'var(--color-text-muted)' }}>{mine ? 'Hikayeye yanıt' : 'Hikayene yanıt'}</span>
                <span style={{ width: 26, height: 40, borderRadius: 6, overflow: 'hidden', flexShrink: 0, border: '1px solid var(--color-border)' }}>
        ...
        <span style={{ fontSize: '0.65rem', color: 'var(--color-text-muted)', whiteSpace: 'nowrap', padding: '0 4px 2px' }}>{timeStr(m.created_at)}</span>
      </div>
    );
  }

  return (
    <main className="dm-main" style={{ display: 'flex', height: 'calc(100dvh)', overflow: 'hidden', flex: 1, background: 'var(--color-bg)' }}>
      {/* Sidebar */}
      <div style={{ width: 320, flexShrink: 0, borderRight: '1px solid var(--color-border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', ...(isNarrow && panelOpen ? { display: 'none' } : {}) }} className="dm-sidebar-col">
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px 12px', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
          <h1 style={{ fontSize: '1.1rem', fontWeight: 700, color: 'var(--color-text)', margin: 0 }}>Mesajlar</h1>
          <button onClick={() => setNewModalOpen(true)} style={{ width: 34, height: 34, borderRadius: '50%', border: '1px solid var(--color-border)', background: 'var(--color-hover)', color: 'var(--color-accent)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'background 0.15s' }}>
        ...
        <div style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'thin' }}>
          {convs.length === 0 ? (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '3rem 1.5rem', gap: 10, color: 'var(--color-text-muted)', textAlign: 'center' }}>
              ...
              <p style={{ fontWeight: 600, color: 'var(--color-text-muted)', margin: 0 }}>Henüz mesajın yok</p>
              <span style={{ fontSize: '0.82rem' }}>Bir kullanıcıya mesaj atmak için profiline git.</span>
            </div>
          ) : convs.map(c => (
            <button key={c.id} onClick={() => openConv(c.id, c.otherUser)} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', width: '100%', background: activeConvId === c.id ? 'var(--color-accent-soft)' : 'none', border: 'none', cursor: 'pointer', textAlign: 'left', transition: 'background 0.15s', borderBottom: '1px solid var(--color-hover)' }}>
              <div style={{ position: 'relative', flexShrink: 0 }}>
                ...
                {c.unreadCount > 0 && <span style={{ position: 'absolute', top: 1, right: 1, width: 11, height: 11, borderRadius: '50%', background: 'var(--color-accent)', border: '2px solid var(--color-bg)' }} />}
              </div>
              <div style={{ flex: 1, minWidth: 0, display: 'flex', flexDirection: 'column', gap: 2 }}>
                <span style={{ fontWeight: 600, fontSize: '0.9rem', color: 'var(--color-text)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{c.otherUser.display_name}</span>
                <span style={{ fontSize: '0.8rem', color: 'var(--color-text-muted)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{msgPreview(c.lastMessage)}</span>
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4, flexShrink: 0 }}>
                {c.lastTimeAgo && <span style={{ fontSize: '0.72rem', color: 'var(--color-text-muted)' }}>{c.lastTimeAgo}</span>}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden', minWidth: 0 }}>
        {!activeConvId ? (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 12, color: 'var(--color-text-muted)', textAlign: 'center', padding: '2rem' }}>
            ...
            <p style={{ fontWeight: 600, color: 'var(--color-text-muted)', margin: 0, fontSize: '1rem' }}>Bir konuşma seç</p>
            <span style={{ fontSize: '0.85rem' }}>Mesajlaşmaya başlamak için soldan bir konuşma seç.</span>
          </div>
        ) : (
          <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
            {/* Header */}
            <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '12px 16px', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                <Link href={`/u/${activeOtherUser?.username}`} style={{ fontWeight: 700, fontSize: '0.95rem', color: 'var(--color-text)', textDecoration: 'none' }}>{activeOtherUser?.display_name}</Link>
                <span style={{ fontSize: '0.78rem', color: 'var(--color-text-muted)' }}>@{activeOtherUser?.username}</span>
              </div>
            </div>

            {/* Messages */}
            <div ref={setMsgBoxRef} style={{ flex: 1, overflowY: 'auto', padding: 16, display: 'flex', flexDirection: 'column', gap: 6, scrollbarWidth: 'thin' }}>
              {msgLoading ? <div style={{ textAlign: 'center', color: 'var(--color-text-muted)', fontSize: '0.85rem', padding: '2rem 0' }}>Mesajlar yükleniyor…</div> : messages.length === 0 ? <div style={{ textAlign: 'center', color: 'var(--color-text-muted)', fontSize: '0.85rem', padding: '2rem 0' }}>Henüz mesaj yok — ilk mesajı sen gönder!</div> : msgElements}
            </div>

            {/* GIF picker */}
            {gifPickerOpen && (
              <div style={{ height: 300, borderTop: '1px solid var(--color-border)', background: 'var(--color-surface)', flexShrink: 0, display: 'flex', flexDirection: 'column' }}>
                <div style={{ padding: '10px 12px 8px', flexShrink: 0, position: 'relative' }}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" style={{ position: 'absolute', left: 22, top: '50%', transform: 'translateY(-50%)', color: 'var(--color-text-muted)', pointerEvents: 'none' }}><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
                  <input value={gifSearch} onChange={e => setGifSearch(e.target.value)} placeholder="GIF ara…" autoComplete="off" style={{ width: '100%', background: 'var(--color-border)', border: '1px solid var(--color-border)', borderRadius: '9999px', padding: '8px 14px 8px 34px', fontSize: '0.85rem', color: 'var(--color-text)', outline: 'none', fontFamily: 'inherit', boxSizing: 'border-box' }} />
                </div>
                <div onScroll={onGifScroll} style={{ flex: 1, overflowY: 'auto', padding: '0 8px 8px', display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 3, alignContent: 'start', scrollbarWidth: 'thin' }}>
                  {gifLoading ? <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '2rem', color: 'var(--color-text-muted)', fontSize: '0.82rem' }}>Yükleniyor…</div> : gifs.length === 0 ? <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '2rem', color: 'var(--color-text-muted)', fontSize: '0.82rem' }}>GIF bulunamadı</div> : gifs.map((gif: any) => {
                    const url = gif.images?.fixed_width?.url ?? gif.images?.original?.url;
                    if (!url) return null;
                    return <button key={gif.id} type="button" onClick={() => sendGif(url)} style={{ position: 'relative', boxSizing: 'content-box', width: '100%', height: 0, paddingBottom: '100%', overflow: 'hidden', borderRadius: 6, cursor: 'pointer', background: 'var(--color-hover)', border: 'none', transition: 'opacity 0.15s' }}><img src={url} alt={gif.title} loading="lazy" style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} /></button>;
                  })}
                  {gifLoadingMore && <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '0.7rem', color: 'var(--color-text-muted)', fontSize: '0.78rem' }}>Daha fazla yükleniyor…</div>}
                </div>
              </div>
            )}

            {/* Input bar */}
            <form onSubmit={sendMsg} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 16px', borderTop: '1px solid var(--color-border)', flexShrink: 0 }}>
              ...
              {mediaEnabled && (
                <>
                  <button type="button" onClick={() => mediaInputRef.current?.click()} disabled={mediaSending} aria-label="Fotoğraf veya video ekle"
                    style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid var(--color-border)', background: 'var(--color-hover)', color: 'var(--color-accent)', cursor: mediaSending ? 'default' : 'pointer', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', opacity: mediaSending ? 0.5 : 1 }}>
              ...
              <button type="button" onClick={() => setGifPickerOpen(p => !p)} style={{ width: 36, height: 36, borderRadius: 8, border: '1px solid var(--color-border)', background: gifPickerOpen ? 'var(--color-accent-soft)' : 'var(--color-hover)', color: 'var(--color-accent)', fontWeight: 800, fontSize: '0.65rem', letterSpacing: '0.02em', cursor: 'pointer', flexShrink: 0, fontFamily: 'inherit' }}>GIF</button>
              <input value={msgText} onChange={e => setMsgText(e.target.value)} placeholder="Mesaj yaz…" maxLength={1000} autoComplete="off" style={{ flex: 1, background: 'var(--color-hover)', border: '1px solid var(--color-border)', borderRadius: '9999px', padding: '10px 16px', fontSize: '0.9rem', color: 'var(--color-text)', outline: 'none', fontFamily: 'inherit' }} />
      ...
      {newModalOpen && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(4px)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={e => { if (e.target === e.currentTarget) { setNewModalOpen(false); setSearchQ(''); setSearchResults([]); } }}>
          <div style={{ background: 'var(--color-surface)', border: '1px solid var(--color-border)', borderRadius: 20, width: '100%', maxWidth: 400, padding: 20, display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontWeight: 700, color: 'var(--color-text)' }}>
              <span>Yeni Mesaj</span>
              <button onClick={() => { setNewModalOpen(false); setSearchQ(''); setSearchResults([]); }} style={{ background: 'none', border: 'none', cursor: 'pointer', color: 'var(--color-text-muted)', padding: 4, borderRadius: '50%', display: 'flex', alignItems: 'center' }}>
            ...
            <input value={searchQ} onChange={e => setSearchQ(e.target.value)} placeholder="Kullanıcı ara…" autoComplete="off" style={{ background: 'var(--color-hover)', border: '1px solid var(--color-border)', borderRadius: '9999px', padding: '10px 16px', fontSize: '0.9rem', color: 'var(--color-text)', outline: 'none', width: '100%', fontFamily: 'inherit', boxSizing: 'border-box' }} />
            <div style={{ maxHeight: 280, overflowY: 'auto', display: 'flex', flexDirection: 'column' }}>
              {searchResults.length === 0 && searchQ && <p style={{ padding: '12px', color: 'var(--color-text-muted)', fontSize: '0.85rem' }}>Kullanıcı bulunamadı</p>}
              {searchResults.map((u: any) => (
                <div key={u.id} role="button" tabIndex={0} aria-label={`${u.display_name ?? u.username} ile konuşma başlat`} onClick={() => startConv(u.username, { id: u.id, username: u.username, display_name: u.display_name ?? u.username, avatar: u.avatar ?? null })} onKeyDown={e => { if (e.key === 'Enter' || e.key === ' ') { e.preventDefault(); startConv(u.username, { id: u.id, username: u.username, display_name: u.display_name ?? u.username, avatar: u.avatar ?? null }); } }} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '10px 4px', cursor: 'pointer', borderRadius: 12, transition: 'background 0.15s' }} onMouseOver={e => (e.currentTarget.style.background = 'var(--color-hover)')} onMouseOut={e => (e.currentTarget.style.background = 'transparent')}>
                  ...
                  <div>
                    <div style={{ fontWeight: 600, fontSize: '0.88rem', color: 'var(--color-text)' }}>{u.display_name ?? u.username}</div>
