...
// Sabit UI iskelesi: üstte okuma-ilerleme çubuğu + kalan süre, solda (masaüstü)
// perde göstergesi. Fatih/Augustus/Kanuni ile aynı sözleşme; yalnız palet ve
// süre farklı. 12 perde → seri içindeki en uzun makale.
  ...
  return (
    <nav aria-label="Perdeler" className="fixed left-4 top-1/2 z-40 hidden -translate-y-1/2 flex-col gap-2.5 lg:flex">
      {items.map((it, i) => {
        ...
        return (
          <a key={it.id} href={`#${it.id}`} className="group flex items-center gap-2" aria-current={on ? 'true' : undefined}>
            <span
