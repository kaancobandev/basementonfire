...
import { OTAG, ONCEKILER, GOC, KAGAN, BARBAR, ISIM, KILIC, HARAC, SURLAR, CATALAUNUM, DEFIN, EFSANE } from './data';

/* ══════════════ Perde 1 · Bozkır zaman şeridi ══════════════ */

/* ══════════════ Perde 0 · Priskos'un sofrası ══════════════ */

// Makalenin en sinematik anı sayfada dört maddelik <ul> olarak duruyordu; oysa
// sahnenin bütün gücü bir KARŞITLIKTA: konukların önünde gümüş tabak ve altın
// kadeh, kağanın önünde tahta tabak ve tahta kadeh — ve çevresindeki beyler
// altın içindeyken o süssüz. Bu karşıtlık söyleniyor, hiç GÖSTERİLMİYORDU.
// Adımlar OTAG.scene ile birebir hizalı; 5. adım OTAG.dress.
export function SofraSahnesi() {
  ...
  useEffect(() => {
    if (!oynuyor) return;
    if (adim >= SON) { setOynuyor(false); return; }
    t.current = setTimeout(() => setAdim((a) => a + 1), 1300);
    return () => { if (t.current) clearTimeout(t.current); };
  }, [oynuyor, adim]);
  useEffect(() => () => { if (t.current) clearTimeout(t.current); }, []);

  const g = reduced ? 0 : 0.55;                 // geçiş süresi
  const KONUK = [78, 132, 186, 240];            // konuk yerlerinin x'i

  return (
    <WidgetFrame
      hero
      kicker={`PERDE 0 · ${OTAG.year} · ${OTAG.place}`}
      title="Sofrada tek bir şey farklı"
      hint="Priskos’un yazdığı sırayla ilerle. Aynı masa, iki ayrı düzen."
      footnote={OTAG.thesis}
    >
      <svg viewBox="0 0 320 200" className="w-full rounded-xl border border-white/10 bg-black/25" role="img"
        aria-label={`Otağda uzun bir masa: konukların önünde gümüş tabak ve altın kadeh, kağanın önünde tahta tabak ve tahta kadeh. Adım ${adim} / ${SON}.`}>
        {/* masa */}
        <rect x="46" y="70" width="228" height="62" rx="7" fill="rgba(255,255,255,0.045)" stroke="rgba(255,255,255,0.12)" />
        <text x="292" y="46" fontSize="7.5" fontWeight="800" letterSpacing="1" fill={ACCENT} textAnchor="end" fontFamily="system-ui, sans-serif">KAĞAN</text>
        <text x="52" y="46" fontSize="7.5" fontWeight="800" letterSpacing="1" fill={BONE} fontFamily="system-ui, sans-serif">KONUKLAR</text>

        {/* konuk yerleri */}
        {KONUK.map((x, i) => (
          <g key={x}>
            {/* 1 · gümüş tabak */}
            <ellipse cx={x} cy="101" rx="17" ry="11" fill="none" stroke={BONE} strokeWidth="1.4"
              style={{ opacity: adim >= 1 ? 0.9 : 0, transition: `opacity ${g}s ease ${i * 0.06}s` }} />
            {/* 2 · altın kadeh */}
            <g style={{ opacity: adim >= 2 ? 1 : 0, transition: `opacity ${g}s ease ${i * 0.06}s` }}>
              <path d={`M${x - 5} 78 L${x + 5} 78 L${x + 3} 87 L${x - 3} 87 Z`} fill={GOLD} opacity="0.85" />
              <rect x={x - 4} y="88" width="8" height="1.6" rx="0.8" fill={GOLD} opacity="0.85" />
            </g>
            {/* 5 · beylerin süsü */}
            <g style={{ opacity: adim >= 5 ? 1 : 0, transition: `opacity ${g}s ease ${i * 0.08}s` }}>
              {[-1, 1].map((s) => <circle key={s} cx={x + s * 22} cy="120" r="2.2" fill={GOLD} opacity="0.75" />)}
              <circle cx={x} cy="124" r="2.2" fill={GARNET} opacity="0.8" />
            </g>
          </g>
        ))}
        ...
        {/* kağanın yeri */}
        <g>
          {/* 3 · tahta tabak — mat, süssüz */}
          <ellipse cx="290" cy="101" rx="16" ry="10.5" fill="rgba(160,130,95,0.16)" stroke="rgba(170,140,100,0.75)" strokeWidth="1.4"
            style={{ opacity: adim >= 3 ? 1 : 0, transition: `opacity ${g}s ease` }} />
          {/* 4 · tahta kadeh — tek başına büyüyor */}
          <g style={{
            opacity: adim >= 4 ? 1 : 0,
            transform: adim >= 4 ? 'translate(290px, 82px) scale(1.35)' : 'translate(290px, 82px) scale(1)',
            transformOrigin: '290px 82px',
            transition: `opacity ${g}s ease, transform ${g * 1.4}s cubic-bezier(0.34,1.4,0.64,1)`,
          }}>
            <path d="M-5 -8 L5 -8 L3 1 L-3 1 Z" fill="rgba(170,140,100,0.32)" stroke="rgba(180,150,110,0.9)" strokeWidth="1.2" />
            <rect x="-4" y="2" width="8" height="1.6" rx="0.8" fill="rgba(180,150,110,0.9)" />
          </g>
          {/* 5 · kağanın çevresi BOŞ kalıyor — karşıtlığın kendisi */}
          <text x="290" y="128" textAnchor="middle" fontSize="7" fontWeight="700" fill={IRON} fontFamily="system-ui, sans-serif"
            style={{ opacity: adim >= 5 ? 1 : 0, transition: `opacity ${g}s ease 0.3s` }}>süs yok</text>
        </g>

        <line x1="262" y1="66" x2="262" y2="136" stroke="rgba(255,255,255,0.1)" strokeDasharray="3 4" />
      </svg>

      {/* Adım metinleri OTAG.scene ile birebir; 5. adım OTAG.dress */}
      <div className="mt-3 min-h-[62px] rounded-xl border border-white/10 bg-black/25 p-3">
        {adim === 0 ? (
          <p className="text-[0.86rem] leading-relaxed text-slate-400">
            {OTAG.witness}, {OTAG.role}. Gördüğü sofrayı yazdı — ve yazdığı şey bir yoksulluk sahnesi değil.
          </p>
        ) : (
          <p className="text-[0.88rem] leading-relaxed text-slate-200" style={{ animation: 'atilla-fade 0.35s ease-out' }}>
            <span className="mr-2 font-mono text-xs font-bold" style={{ color: ACCENT }}>{String(adim).padStart(2, '0')}</span>
            {adim <= 4 ? OTAG.scene[adim - 1] : OTAG.dress}
          </p>
        )}
      </div>

      <div className="mt-3 flex gap-2">
        <ActionButton full onClick={() => { if (adim >= SON) setAdim(0); setOynuyor(!oynuyor); }}>
          {oynuyor ? '⏸ Duraklat' : adim >= SON ? '↺ Baştan' : '▶ Sofrayı kur'}
        </ActionButton>
        <ActionButton tone="ghost" onClick={() => { setOynuyor(false); setAdim(Math.max(0, adim - 1)); }} disabled={adim === 0}>‹</ActionButton>
        <ActionButton tone="ghost" onClick={() => { setOynuyor(false); setAdim(Math.min(SON, adim + 1)); }} disabled={adim >= SON}>›</ActionButton>
      </div>

      {adim >= SON && (
        <p className="mt-4 rounded-xl border p-4 text-[0.95rem] font-semibold leading-relaxed"
          style={{ borderColor: `${ACCENT}55`, background: `color-mix(in srgb, ${ACCENT} 8%, transparent)`, color: '#f5f5f4', animation: 'atilla-fade 0.5s ease-out' }}>
          {OTAG.punch}
        </p>
      )}
    </WidgetFrame>
