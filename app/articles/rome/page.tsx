import type { Metadata } from 'next';
import RomeClient from './RomeClient';

const title = 'Roma İmparatorluğu';
const description = 'Roma\'nın kuruluşundan çöküşüne: On İki Levha Kanunları, Julius Caesar, Augustus, Spartaküs ve daha fazlası — interaktif tarih.';
const path = '/articles/rome';

export const metadata: Metadata = {
  title,
  description,
  alternates: { canonical: path },
  openGraph: { type: 'article', title: `${title} · Basements`, description, url: path, images: ['/opengraph-image'] },
  twitter: { card: 'summary_large_image', title: `${title} · Basements`, description },
};

const jsonLd = {
  '@context': 'https://schema.org',
  '@type': 'Article',
  headline: title,
  description,
  inLanguage: 'tr-TR',
  url: `https://basementonfire.com${path}`,
  author: { '@type': 'Organization', name: 'Basements' },
  publisher: { '@type': 'Organization', name: 'Basements' },
};

export default function Page() {
  return (
    <>
      <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(jsonLd) }} />
      <RomeClient />
    </>
  );
}
