...
const getHomeContent = unstable_cache(
  async () => {
    ...
    const [{ data: rawFacts, error: factsErr }, { data: rawPosts, error: postsErr }, { data: storiesRaw, error: storiesErr }] = await Promise.all([
      ...
      db.from('stories')
        .select('id, media_url, media_type, created_at, user_id, music_track_id, music_start_sec, link_url, link_label, poll_question, poll_options, audience, caption, music:music_tracks(id, title, artist, src), users!stories_user_id_fkey(id, username, display_name, avatar, is_private)')
    ...
    if (storiesErr && /music_track_id|music_start_sec|music_tracks|link_url|link_label|poll_question|poll_options|audience|caption/i.test(storiesErr.message)) {
...
export default async function FeedPage() {
  ...
  interface StoryItem { id: number; mediaUrl: string; mediaType: string; createdAt: string; music?: { title: string; artist: string | null; src: string; startSec: number } | null; linkUrl?: string | null; linkLabel?: string | null; poll?: { question: string; options: string[] } | null; caption?: string | null; seen?: boolean; }
