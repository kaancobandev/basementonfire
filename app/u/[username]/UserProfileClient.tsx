  ...
  return (
    <main className="main-content">
      ...
      <div style={{ padding: '0 20px 16px', borderBottom: '1px solid var(--color-border)' }}>
            ...
            {me ? (
              <>
                <button
                  ...
                  style={{
                    padding: '9px 22px', borderRadius: '9999px', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'inherit',
                ...
                <button
                  onClick={startDm}
                  disabled={dmLoading}
                  style={{ padding: '9px 16px', borderRadius: '9999px', fontSize: '0.9rem', fontWeight: 700, cursor: 'pointer', transition: 'all 0.15s', fontFamily: 'inherit', background: 'transparent', color: 'var(--color-text)', border: '2px solid var(--color-border)', display: 'flex', alignItems: 'center', gap: 6 }}
                >
        ...
        {/* Name + handle */}
        <h1 style={{ fontSize: '1.25rem', fontWeight: 800, margin: '0 0 2px', display: 'flex', alignItems: 'center', gap: 6 }}>
        ...
        <p style={{ color: 'var(--color-text-muted)', fontSize: '0.9rem', margin: '0 0 8px' }}>@{profileUser.username}</p>
        {profileUser.bio && <p style={{ fontSize: '0.9rem', lineHeight: 1.5, margin: '0 0 10px', color: 'var(--color-text)' }}>{profileUser.bio}</p>}
      ...
      {lightbox && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.8)', backdropFilter: 'blur(4px)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={e => { if (e.target === e.currentTarget) closeLightbox(); }}>
          <div style={{ background: 'var(--color-surface)', borderRadius: 16, display: 'flex', maxWidth: 860, width: '100%', maxHeight: '90vh', overflow: 'hidden', position: 'relative' }}>
            ...
            {/* Info panel */}
            <div style={{ width: 280, flexShrink: 0, display: 'flex', flexDirection: 'column', borderLeft: '1px solid var(--color-border)', maxHeight: '90vh' }}>
              {/* User row */}
